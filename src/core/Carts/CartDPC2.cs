...
public sealed class CartDPC2 : Cart
{
    ...
    public override void Reset()
    {
        ...
        _bankBaseAddr = GetBankBaseAddr(5);
    }
    ...
    {
        if (addr is >= 0xff6 and <= 0xffb)
        {
            _bankBaseAddr = GetBankBaseAddr(addr - 0xff6);
        }
    }
    ...
    void WriteReg(ushort addr, byte val)
    {
        ...
        switch (fn)
        {
            case 0x00: // DFxFRACLOW
                _fractionalCounters[i] |= (uint)(val << 8);
                break;
