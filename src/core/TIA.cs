...
public sealed class TIA : IDevice
{
    ...
    void opCTRLPF(ushort addr, byte data)
    {
        ...
        if (data == 0 && (ScanLine == 49 && (PokeOpHSync is 111 or 117 or 123 or 129) || (ScanLine is 259 or 260) && PokeOpHSync == 42))
        {
            // Emulation cheat for Warlords: ignore clearing reflection state in this specific situation
        }
        else
        {
            PFReflectionState = data & 1;
        }
    ...
    void opRESP0(ushort addr, byte data)
    {
        ...
        else if (HMoveLatch && PokeOpHSync is >= 68 and < 76)
        {
    ...
    int SampleINPT(int inpt)
    {
        ...
        switch (inpt <= 1 ? mi.LeftControllerJack : mi.RightControllerJack)
        {
            ...
            case Controller.ProLineJoystick:
                ...
                switch (inpt & 3)
                {
                    case 0: return mi.SampleCapturedControllerActionState(0, ControllerAction.Trigger)  ? 0 : int.MaxValue;
                    case 1: return mi.SampleCapturedControllerActionState(0, ControllerAction.Trigger2) ? 0 : int.MaxValue;
                    case 2: return mi.SampleCapturedControllerActionState(1, ControllerAction.Trigger)  ? 0 : int.MaxValue;
                    case 3: return mi.SampleCapturedControllerActionState(1, ControllerAction.Trigger2) ? 0 : int.MaxValue;
                }
                break;
            case Controller.BoosterGrip:
                // playerno = inpt
                return mi.SampleCapturedControllerActionState(inpt & 3, ControllerAction.Trigger2) ? 0 : int.MaxValue;
    ...
    bool SampleINPTLatched(int inpt, int scanline, int hpos)
    {
        ...
        switch (playerNo == 0 ? mi.LeftControllerJack : mi.RightControllerJack)
        {
            ...
            case Controller.Lightgun:
                mi.SampleCapturedLightGunPosition(playerNo, out var sampledScanline, out var sampledHpos);
    ...
    int SampleKeypadStateDumped(int inpt)
    {
        ...
        // playerno = inpt/2
        return M.InputState.SampleCapturedControllerActionState(inpt >> 1, action) ? int.MaxValue : 0;
    }
